...
class SimulateSEG:
    def __init__(self, processes, memory_size, algorithm):
        ...
        self.memory_map = [{'pid': None, 'start': 0, 'end': memory_size-1, 'segment': None}]  # Initialize memory as entirely free
        self.current_time = 0
        self.input_queue = []
        self.running_processes = []
        self.finished_processes = []
    ...
    def get_mmap(self):
        ...
        for m in self.memory_map:
            if m['pid'] is None:
                ...
                out_mmap.append(f"{m['start']}-{m['end']}: Process {m['pid']}, Segment {m['segment']}")
        return out_mmap
    ...
    def ff_allocate_memory(self, process):
        ...
        for i in range(segs): # Allocate each segment
            for m in temp_mmap:
                if m['pid'] is None and (m['end'] - m['start']) > process.segments[i] - 1:
                    hole = {'pid': None, 'start': m['start'] + process.segments[i], 'end': m['end'], 'segment': None}
                    ...
                    allocated += 1
                    break
                elif m['pid'] is None and (m['end'] - m['start']) == process.segments[i] - 1:
                    m['pid'] = process.pid
                    m['segment'] = i
                    allocated += 1
                    break
    ...
    def bf_allocate_memory(self, process):
        ...
        for j in range(segs): # Allocate each segment
            ...
            i = 0
            for m in temp_mmap:
                if m['pid'] is None and (m['end'] - m['start']) >= process.segments[j] - 1:
                    holes.append([(m['end'] - m['start']), i])
                i += 1
            if holes:
                holes.sort(key=lambda x: x[1])
                ...
                if (best_hole['end'] - best_hole['start']) > process.segments[j] - 1:
                    hole = {'pid': None, 'start': best_hole['start'] + process.segments[j], 'end': best_hole['end'], 'segment': None}
                    temp_mmap.append(hole)
                    temp_mmap[holes[0][1]]['pid'] = process.pid
                    temp_mmap[holes[0][1]]['end'] = best_hole['start'] + process.segments[j] - 1
...
def seg_test():
    processes = read_workload_file("data/test02/input.txt")
